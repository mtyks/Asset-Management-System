...
export default function AssetDetail() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setAsset(a);
      const [h, b] = await Promise.all([getAssetHistory(a.asset_code), getActiveBorrowRecord(a.asset_code)]);
      setHistory(h);
      setActiveBorrow(b);
    } catch (err) {
  ...
  async function handleStatusChange(newStatus) {
    setBusy(true);
    try {
      await updateAssetStatus(asset.asset_code, newStatus);
      await load();
    } catch (err) {
  ...
  async function handleBorrow(e) {
    ...
    setBusy(true);
    try {
      await borrowAsset(asset.asset_code, borrowerName, borrowerContact, dueDate || null);
      setBorrowerName("");
      setBorrowerContact("");
      setDueDate("");
      await load();
  ...
  async function handleSaveResponsible() {
    setBusy(true);
    try {
      await updateAsset(asset.asset_code, { responsible_person: responsibleDraft || null });
  ...
  async function handleReturn() {
    ...
    setBusy(true);
    try {
      await returnAsset(asset.asset_code, activeBorrow.id);
      await load();
    } catch (err) {
      setError(err.message);
    } finally {
    ...
    <div className="page asset-detail">
      ...
      {user && (
        <div className="form-actions" style={{ marginBottom: 16 }}>
          <Link to={`/assets/${asset.asset_code}/edit`} className="btn btn-secondary">
            แก้ไขข้อมูลทั้งหมด
          </Link>
        </div>
      )}
